...
class PromptToSlideAgent:
    ...
    def generate_slides(self, 
        ...
        Args:
            ...
            skip_db: If True, return data without storing to database
            
        Returns:
            Dict with generated slide deck and metadata
        """
        try:
            # Store original prompt (or prepare data if skip_db)
            if skip_db:
                prompt_data = self._prepare_prompt_data(prompt_text, user_id, locale, context)
                prompt_id = None  # Will be generated by Go
            else:
            ...
            # Store generated slides with analysis for category tagging (or prepare data if skip_db)
            if skip_db:
                deck_data = self._prepare_slide_deck_data(prompt_id, slide_deck, user_id, context, analysis=analysis)
                deck_id = None  # Will be generated by Go
            else:
    ...
    def _prepare_prompt_data(self, prompt_text: str, user_id: str, locale: str, context: Optional[Dict]) -> Dict:
        """Prepare prompt data without storing to database"""
        return {
            "userId": user_id,
            "prompt_text": prompt_text,
            "timestamp": datetime.utcnow(),
...
{{
  ...
  "target_audience": "audience level"
    ...
    def _prepare_slide_deck_data(self, prompt_id: str, slide_deck: SlideDeck, user_id: str, context: Optional[Dict] = None, analysis: Optional[Dict] = None) -> Dict:
        """Prepare slide deck data without storing to database"""
        # Get analysis from context if not provided
        if analysis is None:
            analysis = {}
